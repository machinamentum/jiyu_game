...
struct Tunnelling : BasicVR
{
    ...
    void MainLoop()
    {
		...
		ovrFovPort newFov[2];
		newFov[0].UpTan = std::max(HmdDesc.DefaultEyeFov[0].UpTan, HmdDesc.DefaultEyeFov[1].UpTan);
		newFov[0].DownTan = std::max(HmdDesc.DefaultEyeFov[0].DownTan, HmdDesc.DefaultEyeFov[1].DownTan);
		newFov[0].LeftTan = std::max(HmdDesc.DefaultEyeFov[0].LeftTan, HmdDesc.DefaultEyeFov[1].LeftTan);
		newFov[0].RightTan = std::max(HmdDesc.DefaultEyeFov[0].RightTan, HmdDesc.DefaultEyeFov[1].RightTan);
		newFov[1] = newFov[0];
	    ...
	    while (HandleMessages())
	    {
			...
			marginx = std::min(1.0f,marginx);
			marginx = std::max(0.0f,marginx);
			marginy = std::min(1.0f,marginy);
			marginy = std::max(0.0f,marginy);
		    ...
		    for (int eye = 0; eye < 2; ++eye)
		    {
				...
				proportionOfStatic = std::min(1.0f,proportionOfStatic);
				proportionOfStatic = std::max(0.0f,proportionOfStatic);
