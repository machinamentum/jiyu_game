...
struct ZoomForIO : BasicVR
{
    ...
    void MainLoop()
    {
        ...
        auto zoomedTexture = new Texture(true, std::max(Layer[0]->pEyeRenderTexture[0]->SizeW, Layer[0]->pEyeRenderTexture[1]->SizeW),
                                               std::max(Layer[0]->pEyeRenderTexture[0]->SizeH, Layer[0]->pEyeRenderTexture[1]->SizeH));
	    ...
	    while (HandleMessages())
	    {
            ...
            if (DIRECTX.Key['1']) amountOfZoom = std::max(amountOfZoom - 0.002f, 0.050f);
            if (DIRECTX.Key['2']) amountOfZoom = std::min(amountOfZoom + 0.002f, 0.500f);
		    ...
		    for (int eye = 0; eye < 2; ++eye)
		    {
                ...
                if (DIRECTX.Key['3']) howFarAway = std::max(howFarAway - 0.002f, 0.25f);
                if (DIRECTX.Key['4']) howFarAway = std::min(howFarAway + 0.002f, 1.00f);
