...
struct BufferedHaptics : BasicVR
{
	...
	void MainLoop()
	{
		...
		if (desc.SampleSizeInBytes != 1) {
		  FATALERROR("Our assumption of 1 byte per element, is no longer valid");
		}
		if (desc.SubmitMinSamples > bufferSize) {
		  FATALERROR("Can't handle this few samples");
		}
		if (desc.QueueMinSizeToAvoidStarvation > bufferSize) {
		  FATALERROR("Not enough samples to avoid starvation");
		}
		if (desc.SubmitMaxSamples < bufferSize) {
		  FATALERROR("Can't handle this many samples");
		}
