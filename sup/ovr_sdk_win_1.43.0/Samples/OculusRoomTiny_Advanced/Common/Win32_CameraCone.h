...
struct CameraCone
{
    ...
    float DistToBoundary(XMVECTOR centreEyePosePos, ovrPosef cameraPose)
    {
        ...
        dist = std::min(dist, DistToPlane(&centreEyePosePos, &v[5], &v[6], &v[4]));// Back
        dist = std::min(dist, DistToPlane(&centreEyePosePos, &v[4], &v[2], &v[0]));// Left
        dist = std::min(dist, DistToPlane(&centreEyePosePos, &v[1], &v[7], &v[5]));// Right
        dist = std::min(dist, DistToPlane(&centreEyePosePos, &v[4], &v[1], &v[5]));// Top
        dist = std::min(dist, DistToPlane(&centreEyePosePos, &v[2], &v[7], &v[3]));// Bottom
        return(dist);
    }
