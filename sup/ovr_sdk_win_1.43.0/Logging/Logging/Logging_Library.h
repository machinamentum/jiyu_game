...
{
   return path[index] ? ovrlog::LoggingFileLineHash(path, index + 1, (((uint8_t)path[index] ^ hash) * LOGGING_FILE_LINE_FNV_HASH_PRIME)) : hash;
}
  ...
  #define LOGGING_LOCATION_HASH() \
      ...
      ovrlog::LoggingFileLineHash(__FILE__, 0, (uint64_t)((__LINE__ ^ LOGGING_FILE_LINE_FNV_HASH_OFFSET) * LOGGING_FILE_LINE_FNV_HASH_PRIME)) \
...
public:
    ...
    //
    void SetOutputPluginChannels(const char* outputPluginName, const std::vector<std::string>& channelNames);

    // Sets the set of outputs that a given channel can write to.
    void SetChannelOutputPlugins(const char* channelName, const std::vector<std::string>& outputPluginNames);
...
public:
    ...
    static void SetConfiguratorOnChannelLevelChange(ConfiguratorOnChannelLevelChangeType function)
      { ConfiguratorOnChannelLevelChange = function; }

    // Target of Register
    static void SetConfiguratorRegister(ConfiguratorRegisterType function)
      { ConfiguratorRegister = function; }

    // Target of Unregister
    static void SetConfiguratorUnregister(ConfiguratorUnregisterType function)
      { ConfiguratorUnregister = function; }
