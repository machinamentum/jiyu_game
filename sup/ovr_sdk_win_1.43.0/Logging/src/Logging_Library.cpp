...
void OutputWorker::SetOutputPluginChannels(const char* outputPluginName, const std::vector<std::string>& channelNames)
{
    Locker locker(PluginsLock);
...
void OutputWorker::SetChannelOutputPlugins(const char* channelName, const std::vector<std::string>& outputPluginNames)
{
    Locker locker(PluginsLock);
    ...
    if (outputPluginNames.empty())
        ...
        ChannelOutputPluginFiltering[channelName].insert(outputPluginNames.begin(), outputPluginNames.end());
    }
}
...

    {
        ...
        for (QueuedLogMessage* next; message; message = next)
        {
            ...
            else
            {
                ...
                AppendHeader(HeaderBuffer + timestampLength, sizeof(HeaderBuffer) - timestampLength, level, subsystemName);
                ...
                for (auto& pluginIt : Plugins) {
                    ...
                    bool channelWritesToPlugin = (itC == ChannelOutputPluginFiltering.end()); // Typically true.
                    ...
                    if (channelWritesToPlugin) { // Typically true.
                        ...
                        bool pluginAllowsChannel = (itO == OutputPluginChannelFiltering.end()); // Typically true.
...
// Should be locked already when calling this function!
void Configurator::SetChannelLevelNoLock(const std::string& channelName, Level level, bool overrideUser)
{
    for (ChannelNode* channelNode = ChannelNodeHead; channelNode; channelNode = channelNode->Next)
    {
...
void Configurator::SetOutputPluginChannels(const char* outputPluginName, const std::vector<std::string>& channelNames)
{
    OutputWorker::GetInstance()->SetOutputPluginChannels(outputPluginName, channelNames);
}

void Configurator::SetChannelOutputPlugins(const char* channelName, const std::vector<std::string>& outputPluginNames)
{
